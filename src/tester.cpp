...
class StdinGraphSource : public AbstractGraphSource {
     ...
     */
    static int graphsize(const char *s) {
        const char *p;
        int n;
 ...
 public:
    ...
    bool get(Graph &G) {
        ...
        if (r) {
            const char *cp = reinterpret_cast<const char*>(s.c_str());
            stringtograph(cp, G);
        }
...
class TestRunner {
 ...
 public:
    TestRunner(int minC, int maxC, int cutSizeB,
               unique_ptr<AbstractGraphSource> graphs)
        : graphs(move(graphs)), minComponents(minC),
          maxComponents(maxC), cutSizeBound(cutSizeB)
    { }
